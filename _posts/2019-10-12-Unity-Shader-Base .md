...
- 渲染流水线
    ...
    - 几何阶段
        ...
        - 屏幕映射
            ...
            > ![10.png](http://ww1.sinaimg.cn/large/006zwgbUly1gdt4vbpkabj30hp05it9c.jpg)&emsp;&emsp;将齐次坐标下 -1，1的坐标范围转换到（x1,y1）,(x2,y2)。可以看到这个过程实际上就是一个缩放的过程。在这个处理种z轴不做处理。屏幕坐标系和z轴构成了窗口坐标系。这些值会被传到光栅化阶段
    ...
    - 光栅化阶段
        ...
        - 逐片元操作
            ...
            以上顺序不是固定的，对于大多数GPU来说，会尽可能在片元着色器之前进行测试，以避免做无用功（计算了片元信息后而没有通过测试舍弃该片元的情况），浪费性能；
...
- 说明
    ...
    > &emsp;&emsp;unity-shader并不是真正的shader，unity-shader是用ShaderLab语言来编写的，S**haderLab语言是unity封装的，其是GLSL,HLSL,CG的上层语言**，其内部真正的着色器代码还是**从属于CG语言**的，着色器代码嵌套在**CGPROGRAM和ENDCG**之间
